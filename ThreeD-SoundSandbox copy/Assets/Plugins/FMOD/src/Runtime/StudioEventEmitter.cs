...
namespace FMODUnity
{
    ...
    public class StudioEventEmitter : EventHandler
    {
        ...
        private float _currentOcclusionTarget = 0.0f;
        ...
        void Update()
        {
            if (instance.isValid())
            {
                if (!occlusionEnabled)
                {
                    _currentOcclusionTarget = 0.0f;
                }
                else if (Time.time >= _nextOcclusionUpdate)
                {
                    _nextOcclusionUpdate = Time.time + FmodResonanceAudio.occlusionDetectionInterval;
                    _currentOcclusionTarget = occlusionIntensity * FmodResonanceAudio.ComputeOcclusion(transform);
                }
                else
                {
                    _currentOcclusionValue =
                        Mathf.MoveTowards(_currentOcclusionValue, _currentOcclusionTarget, Time.deltaTime * 2);
                    instance.setParameterByName(occlusionParameterName, _currentOcclusionValue);
                }
            }
        }
    }
