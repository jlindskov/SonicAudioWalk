...
public static class FmodResonanceAudio
{
    ...
    private static Transform _listenerTransform = null;
    ...
    public static float ComputeOcclusion(Transform sourceTransform)
    {
        ...
        if (ListenerTransform != null)
        {
            ...
            Debug.DrawLine(listenerPosition, sourceTransform.position);
            ...
            {
                if (!occlusionHits[i].collider.isTrigger && occlusionHits[i].transform != _listenerTransform && occlusionHits[i].transform != sourceTransform)
                {
                    Debug.DrawLine(listenerPosition, occlusionHits[i].point, Color.red);
                    occlusion += 1.0f;
                }
            }
        }
